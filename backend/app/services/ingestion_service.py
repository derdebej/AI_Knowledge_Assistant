"""Extract -> clean -> chunk pipeline. See specs/RAG_PIPELINE.md §1 and
specs/ARCHITECTURE.md §5.1.

Embedding generation is Phase 3 - a document processed here reaches
`completed` once its chunks are persisted, per specs/ROADMAP.md Phase 2
("embedding step stubbed/skipped until Phase 3").
    ...
    async def process(self, document_id: uuid.UUID, *, user_id: uuid.UUID) -> None:
        async with self._session_factory() as session:
            ...
            try:
                ...
                await chunk_repository.replace_all_for_document(document_id, renumbered_chunks)
                await document_repository.update_status(
                    document_id,
                    user_id=user_id,
